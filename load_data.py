...
link_patients = "https://api.nabla.com/v1/server/patients"


def save_patient(data):
    for patient in data:
        s3.put_object(
            Bucket="nablalog", Key=f"patients/{patient['id']}.json", Body=json.dumps(patient)
        )
...
link_providers = "https://api.nabla.com/v1/server/providers"


def save_providers(data):
    for patient in data:
        s3.put_object(
            Bucket="nablalog", Key=f"prodviders/{patient['id']}.json", Body=json.dumps(patient)
        )
