...
def get_files(bucket_name,prefix,s3):
    ...
    while s3_result['IsTruncated']:
        ...
        s3_result = s3.list_objects_v2(Bucket=bucket_name, Prefix=prefix, Delimiter="/", ContinuationToken=continuation_key)
        for key in s3_result['Contents']:
            file_list.append(key['Key'])
        print(f"List count = {len(file_list)}")
    return file_list
...
def get_data_msg(key):
    ...
    result = json.loads(result)
    try:
        return {'id':result['id'], 'created_at':result['created_at'], 'author_id' : result['data']['author']['id'],
                'author_type' : result['data']['author']['type'], 'conversation_id':result['data']['conversation_id']}
    except:
        pass
...
def get_data_video(key):
    ...
    result = json.loads(result)
    try:
        return {'id':result['id'], 'start_at':result['data']['start_at'], 'patient' : result['data']['patient']['id'],
                'provider' : result['data']['provider']['id'], 'finish_at':result['created_at']}
    except:
        pass
...
def get_data_conversation(key):
    ...
    result = json.loads(result)
    try:
        return {'conversation_id' : result['data']['id'],'patient_id' :result['data']['patients'][0]['id']}
    except:
        pass
...
def get_patients():
    ...
    patient_base = pd.read_csv(s3.get_object(Bucket='nablaanalyticsv0', Key='patients.csv')['Body'])
    patient_id_loaded = [True if item not in patient_base.id.values else False for item in patient_id_loaded ]
...
def get_providers():
    ...
    providers_base = pd.read_csv(s3.get_object(Bucket='nablaanalyticsv0', Key='providers.csv')['Body'])
    providers_id_loaded = [True if item not in providers_base.id.values else False for item in providers_id_loaded ]
...
def get_video():
    ...
    video_id_loaded = [True if item not in video_base.id.values else False for item in video_id_loaded]
...
def get_conversation():
    ...
    conv_base = pd.read_csv(s3.get_object(Bucket='nablaanalyticsv0', Key='conversations.csv')['Body'])
    conv_id_loaded = [True if item not in conv_base.conversation_id.values else False for item in conv_id_loaded]
